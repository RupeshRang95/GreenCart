import React, { useState, useRef, useMemo, Suspense, lazy, useCallback, useEffect } from "react";
import {
  Aperture,
  CloudUpload,
  ChevronDown,
...
const processingSteps = [
  { step: 1, label: "Reading receipt...", sub: "Tesseract OCR → line normalization", Icon: ScanSearch },
  { step: 2, label: "Identifying products...", sub: "Name matching → Open Food Facts lookup", Icon: ClipboardCheck },
  { step: 3, label: "Looking up origins...", sub: "Brand DB → produce DB → category heuristic", Icon: Globe2 },
  { step: 4, label: "Calculating impact...", sub: "Poore & Nemecek emission factors → ESG score", Icon: BarChart4 },
];

const TRANSPORT_META: Record<string, { icon: React.FC<{ size?: number; className?: string }>; label: string; color: string }> = {
...
function buildTransportBreakdown(items: ScannedItem[]) {
  const map: Record<string, number> = {};
  for (const item of items) map[item.transport] = (map[item.transport] ?? 0) + 1;
...
const ScannerScreen: React.FC = () => {
  ...
  const [tripMeta, setTripMeta] = useState(() => aggregateTripScore(sampleReceiptItems));
  ...
  const [barcodeFeedback, setBarcodeFeedback] = useState<{ kind: "ok" | "err"; text: string } | null>(null);
  ...
  const [celebration, setCelebration] = useState<{ breakdown: XPBreakdown; totalXP: number; streak: number } | null>(null);

  const [captureMode, setCaptureMode] = useState<string | undefined>(undefined);
  const fileRef = useRef<HTMLInputElement>(null);

  // Load history on mount
  useEffect(() => { setHistory(loadTripHistory()); }, []);
  ...
  const finalizeResults = useCallback((items: ScannedItem[]) => {
    ...
    setTripMeta(meta);
    const trip = saveTripToHistory(items, { score: meta.score, grade: meta.grade });
    setSavedTrip(trip);
    ...
    const breakdown = processTripXP(items, { score: meta.score, grade: meta.grade });
    const gs = loadGamificationState();
    setCelebration({ breakdown, totalXP: gs.xp, streak: gs.streak });
  }, []);

  const applyFallbackDemo = (msg: string) => {
    toast.message(msg, { description: "Showing demo trip data so you can explore the UI." });
  ...
  const runPipeline = async (fn: () => Promise<ScannedItem[]>) => {
    ...
    try {
      ...
      if (merged.length === 0) { applyFallbackDemo("No items found."); return; }
      finalizeResults(merged);
  ...
  const onFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    await runPipeline(() =>
      runReceiptPipeline(file, {
        onStep: (step, detail) => { setProcessingStep(step); setProcessingDetail(detail); },
      })
    );
  };
  ...
  const lookupBarcode = async () => {
    ...
    if (code.length < 6) {
      setBarcodeFeedback({ kind: "err", text: "Enter at least 6 digits from the package barcode." });
      return;
    }
    setBarcodeBusy(true);
    setBarcodeFeedback(null);
    try {
      ...
      if (!item) {
        setBarcodeFeedback({ kind: "err", text: "No match found. Try a national brand barcode (e.g. from Burnbrae, Chiquita, Natrel)." });
      ...
      const msg = e instanceof Error ? e.message : "Lookup failed (network or blocked).";
  ...
  const onBarcodeDetected = async (code: string) => {
    ...
    try {
      const item = await itemFromBarcode(code);
      if (!item) {
        setBarcodeFeedback({ kind: "err", text: "No match found. Try another barcode or type it manually." });
      ...
      setBarcodeFeedback({ kind: "err", text: e instanceof Error ? e.message : "Lookup failed." });
      toast.error("Barcode lookup failed");
    } finally {
      setBarcodeBusy(false);
    }
  ...
  const displayItems = phase === "results" && receiptItems.length > 0 ? receiptItems
    : historyItem ? historyItem.items
    : sampleReceiptItems;

  const categoryBreakdown = useMemo(() => buildCategoryBreakdown(displayItems), [displayItems]);
  const transportBreakdown = useMemo(() => buildTransportBreakdown(displayItems), [displayItems]);
  const totalCO2g = useMemo(() => displayItems.reduce((s, i) => s + i.co2, 0), [displayItems]);
  const pctLocal = useMemo(
    () => Math.round((displayItems.filter((i) => i.origin.distance < 200).length / Math.max(displayItems.length, 1)) * 100),
    [displayItems]
  );
  const totalSpent = useMemo(() => displayItems.reduce((s, i) => s + i.price, 0), [displayItems]);
  const worstItems = useMemo(() => [...displayItems].sort((a, b) => b.co2 - a.co2).slice(0, 3), [displayItems]);
  const bestItems = useMemo(() => [...displayItems].sort((a, b) => a.co2 - b.co2).slice(0, 3), [displayItems]);

  const activeScore = phase === "results" ? tripMeta.score
    : historyItem ? historyItem.esgScore
    : 642;
  const activeGrade = phase === "results" ? tripMeta.grade
    : historyItem ? historyItem.grade
    : "B-";
    ...
    <div className="flex flex-col h-full bg-background">
      ...
      <Dialog open={pasteOpen} onOpenChange={setPasteOpen}>
        <DialogContent className="max-w-[min(100vw-2rem,420px)] border-border bg-card">
          <DialogHeader>
            <DialogTitle className="font-display">Paste order confirmation</DialogTitle>
          </DialogHeader>
          <Textarea
            value={pasteText}
      ...
      <div className="px-5 pt-14 pb-3 flex items-center justify-between">
        <div>
          ...
          <p className="text-[12px] text-foreground-tertiary">
            {phase === "history" ? `${history.length} saved trips` : "Snap your receipt to see the true cost"}
          </p>
        </div>
        ...
        {phase === "history" && (
          <button type="button" onClick={() => setPhase("idle")} className="text-[12px] text-primary font-medium">
            Done
          </button>
        )}
      </div>
          ...
          <div className="animate-fade-up space-y-3">
            ...
            <div className="card-surface overflow-hidden">
              <div className="flex flex-col items-center py-8 px-4">
                ...
                <p className="text-[15px] font-display font-bold text-foreground mb-1">Scan your receipt</p>
                <p className="text-[11px] text-foreground-tertiary mb-5 text-center">
                  Canadian grocery receipts (Walmart, FreshCo, No Frills, Sobeys, and more)
                </p>
              ...
              <div className="px-4 py-3">
                ...
                >
                  ...
                  {barcodeOpen ? <Minus size={14} className="text-foreground-tertiary" /> : <Plus size={14} className="text-foreground-tertiary" />}
                </button>
                ...
                {barcodeOpen && (
                  <div className="mt-3 space-y-2 animate-fade-up">
                    <div className="flex gap-2">
                      ...
                      >
                        {barcodeBusy ? <Loader2 size={13} className="animate-spin" /> : null}
                        Add
                      </button>
                    </div>
                    ...
                    {barcodePending.length > 0 && (
                      <div className="flex flex-wrap gap-1.5">
                        {barcodePending.map((b, i) => (
                          <span key={b.id} className="text-[10px] px-2 py-1 rounded-lg bg-primary/10 text-primary font-medium flex items-center gap-1">
                            {b.name}
                            <button
                              type="button"
                              onClick={() => setBarcodePending((p) => p.filter((_, j) => j !== i))}
                    ...
                    {barcodePending.length > 0 && (
                      ...
                      >
                        Analyze {barcodePending.length} item{barcodePending.length !== 1 ? "s" : ""} →
                      </button>
                    )}
                  </div>
                )}
            ...
            {history.length > 0 && (
              <div>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-[12px] font-display font-semibold text-foreground">Recent trips</p>
                  {history.length > 3 && (
                    <button type="button" onClick={() => setPhase("history")} className="text-[11px] text-primary">
                      See all →
                    </button>
                  )}
                </div>
                <div className="space-y-2">
                  {history.slice(0, 3).map((trip) => (
                    <TripHistoryCard
                      key={trip.id}
                      trip={trip}
                      onOpen={() => { setHistoryItem(trip); setPhase("results"); }}
        ...
        {phase === "processing" && (
          <div className="animate-fade-up space-y-4">
            <div className="card-surface flex flex-col items-center py-8">
              ...
              {processingDetail && (
                <p className="text-[10px] text-primary/90 font-mono text-center mb-4 px-2">{processingDetail}</p>
              )}
                ...
                {processingSteps.map(({ step, label, sub, Icon }) => {
                  ...
                  return (
                    <div key={step} className={`flex items-center gap-3 transition-all duration-500 ${processingStep >= step ? "opacity-100" : "opacity-25"}`}>
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 transition-all ${done ? "bg-primary/20" : active ? "bg-primary/10 animate-glow-pulse" : "bg-card-elevated"}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className={`text-[13px] font-medium transition-colors ${active ? "text-foreground" : done ? "text-foreground-secondary" : "text-foreground-tertiary"}`}>{label}</div>
                        <div className="text-[10px] text-foreground-tertiary font-mono truncate">{active && processingDetail ? processingDetail : sub}</div>
                      </div>
                      {active && <div className="w-4 h-4 rounded-full border-2 border-primary border-t-transparent animate-spin shrink-0" />}
                    </div>
                  );
                })}
              </div>
          ...
          <div className="animate-fade-up space-y-3">
            ...
            {savedTrip && (
              <div className="flex items-center gap-2 px-3 py-2.5 rounded-2xl bg-primary/10 border border-primary/20">
                ...
                <button type="button" onClick={resetScan} className="flex items-center gap-1 text-[10px] text-primary/80 font-medium">
            ...
            {!savedTrip && historyItem && (
              <div className="flex items-center gap-2 px-3 py-2.5 rounded-2xl bg-card-elevated border border-border">
                <Clock size={13} className="text-foreground-tertiary shrink-0" />
                <span className="text-[11px] text-foreground-secondary flex-1">{historyItem.displayDate}</span>
                <button type="button" onClick={() => { setHistoryItem(null); setPhase("history"); }} className="text-[10px] text-primary font-medium">
                  ← History
                </button>
              </div>
            )}
            ...
            <div className="card-surface flex flex-col items-center py-5">
              <ESGGauge score={activeScore} grade={activeGrade} size={170} change={activeChange} />
              {activeChange !== 0 && (
                <div className={`flex items-center gap-1 text-[11px] font-medium mt-1 ${activeChange > 0 ? "text-primary" : "text-destructive"}`}>
                  {activeChange > 0 ? <TrendingUp size={12} /> : <TrendingDown size={12} />}
                  {activeChange > 0 ? "+" : ""}{activeChange} pts from last trip
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {[
                { label: "Items", value: displayItems.length.toString(), Icon: ShoppingCart },
                { label: "Local", value: `${pctLocal}%`, Icon: MapPinned },
                { label: "CO₂", value: `${(totalCO2g / 1000).toFixed(1)}kg`, Icon: Sprout },
                { label: "Spent", value: `$${totalSpent.toFixed(0)}`, Icon: Banknote },
              ].map((m) => (
                <div key={m.label} className="card-surface p-2.5 flex flex-col items-center text-center">
                  ...
                  <span className="font-mono font-bold text-[14px] text-foreground leading-tight">{m.value}</span>
                  <span className="text-[9px] text-foreground-tertiary">{m.label}</span>
                </div>
              ))}
            </div>
            ...
            <div className="card-surface p-4">
              <p className="text-[12px] font-display font-semibold text-foreground mb-3">Transport modes</p>
              <div className="flex flex-wrap gap-2">
                {transportBreakdown.map(([mode, count]) => {
                  const meta = TRANSPORT_META[mode] ?? TRANSPORT_META.truck;
                  const TIcon = meta.icon;
                  return (
                    <div key={mode} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-background-tertiary border border-border">
                      <TIcon size={12} className={meta.color} />
                      <span className="text-[11px] text-foreground-secondary font-medium capitalize">{meta.label}</span>
                      <span className="text-[11px] text-foreground-tertiary">· {count} item{count !== 1 ? "s" : ""}</span>
                    </div>
                  );
                })}
              </div>
              {transportBreakdown.some(([m]) => m === "air") && (
                <p className="mt-2 text-[10px] text-red-400/80 leading-snug">
                  ✈️ Air-freighted items have 50× higher transport emissions than ship.
                </p>
              )}
            </div>
            ...
            {/* Carbon by category */}
            <div className="card-surface p-4">
              <p className="text-[12px] font-display font-semibold text-foreground mb-3">Carbon by category</p>
                ...
                {categoryBreakdown.map((c) => (
                  <div key={c.cat}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-[11px] font-mono text-foreground-tertiary">{c.co2kg} kg</span>
                    </div>
                    <div className="h-1.5 rounded-full bg-background-tertiary overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${c.pct}%`,
                          background: c.pct > 35 ? "hsl(0 72% 71%)" : c.pct > 20 ? "hsl(43 96% 56%)" : "hsl(142 69% 58%)",
                        }}
                      />
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="card-surface p-3">
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <p className="text-[11px] font-display font-semibold text-foreground">Low impact</p>
                </div>
                <div className="space-y-1.5">
                  {bestItems.map((item) => (
                    <div key={item.id} className="flex items-center justify-between gap-1">
                      <span className="text-[10px] text-foreground-secondary truncate flex-1">{item.name}</span>
                      <span className="text-[10px] font-mono text-primary shrink-0">{item.co2}g</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="card-surface p-3">
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <p className="text-[11px] font-display font-semibold text-foreground">High impact</p>
                </div>
                <div className="space-y-1.5">
                  {worstItems.map((item) => (
                    <div key={item.id} className="flex items-center justify-between gap-1">
                      <span className="text-[10px] text-foreground-secondary truncate flex-1">{item.name}</span>
                      <span className="text-[10px] font-mono text-destructive shrink-0">{(item.co2 / 1000).toFixed(1)}kg</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {displayItems.some((i) => i.localAlt) && (
              <div className="card-surface p-4">
                <p className="text-[12px] font-display font-semibold text-foreground mb-3">💡 Swap suggestions</p>
                <div className="space-y-2.5">
                  {displayItems.filter((i) => i.localAlt).slice(0, 3).map((item) => (
                    <div key={item.id} className="flex items-center gap-3 p-2.5 rounded-xl bg-background-tertiary">
                      <div className="flex-1 min-w-0">
                        <div className="text-[10px] text-foreground-tertiary">Instead of</div>
                        <div className="text-[11px] font-semibold text-foreground truncate">{item.name}</div>
                        <div className="text-[10px] text-foreground-tertiary">{item.co2}g CO₂</div>
                      </div>
                      <MoveRight size={14} className="text-primary shrink-0" />
                      <div className="flex-1 min-w-0 text-right">
                        <div className="text-[10px] text-primary">Try local</div>
                        <div className="text-[11px] font-semibold text-foreground truncate">{item.localAlt!.name}</div>
                        <div className="text-[10px] text-primary">{item.localAlt!.co2}g · -{Math.round(((item.co2 - item.localAlt!.co2) / item.co2) * 100)}% CO₂</div>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {/* Item breakdown */}
            <div>
              <p className="text-[12px] font-display font-semibold text-foreground mb-2">All items ({displayItems.length})</p>
              <div className="space-y-2">
          ...
          <div className="animate-fade-up space-y-2">
            {history.length === 0 ? (
              <div className="flex flex-col items-center py-16 gap-3">
                ...
                <p className="text-[13px] text-foreground-tertiary">No trips saved yet</p>
                <button type="button" onClick={resetScan} className="text-[12px] text-primary font-medium">
              ...
              history.map((trip) => (
                <TripHistoryCard
                  key={trip.id}
                  trip={trip}
                  onOpen={() => { setHistoryItem(trip); setSavedTrip(null); setPhase("results"); }}
                  onDelete={() => deleteHistory(trip.id)}
                  expanded
                />
              ))
      ...
      <button type="button" onClick={onOpen} className="w-full text-left">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center shrink-0`}>
            <span className={`text-[15px] font-display font-bold ${gc}`}>{trip.grade}</span>
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <span className="text-[13px] font-semibold text-foreground">{trip.displayDate}</span>
              {trip.store && <span className="text-[10px] text-foreground-tertiary">· {trip.store}</span>}
            </div>
            <div className="text-[11px] text-foreground-tertiary">
              {trip.itemCount} items · ${trip.totalSpent.toFixed(2)} · {(trip.co2TotalG / 1000).toFixed(2)} kg CO₂
            </div>
          </div>
          {expanded && (
            <div className="flex items-center gap-2 shrink-0">
              ...
              <ChevronDown size={14} className="text-foreground-tertiary -rotate-90" />
            </div>
          )}
        </div>
        ...
        {expanded && (
          <div className="mt-2.5 flex gap-2">
            <div className="flex-1 px-2.5 py-1.5 rounded-xl bg-background-tertiary text-center">
              <div className="text-[12px] font-mono font-bold text-foreground">{trip.pctLocal}%</div>
              <div className="text-[9px] text-foreground-tertiary">Local</div>
            </div>
            <div className="flex-1 px-2.5 py-1.5 rounded-xl bg-background-tertiary text-center">
              <div className="text-[12px] font-mono font-bold text-foreground">{(trip.co2TotalG / 1000).toFixed(2)}</div>
              <div className="text-[9px] text-foreground-tertiary">kg CO₂</div>
            </div>
            <div className="flex-1 px-2.5 py-1.5 rounded-xl bg-background-tertiary text-center">
              <div className={`text-[12px] font-display font-bold ${gc}`}>{trip.esgScore}</div>
...
const ItemCard: React.FC<{
  ...
  const co2Color = item.co2 > 2000 ? "text-destructive" : item.co2 > 800 ? "text-warning" : "text-primary";
    ...
    <div className="card-surface overflow-hidden">
      <button type="button" onClick={onToggle} className="w-full text-left">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center shrink-0">
            {CategoryIcon ? <CategoryIcon size={14} className="text-primary" /> : null}
          </div>
          <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <div className="text-right">
              <div className={`text-[12px] font-mono font-bold ${co2Color}`}>{item.co2 >= 1000 ? `${(item.co2/1000).toFixed(1)}kg` : `${item.co2}g`}</div>
              <div className="text-[9px] text-foreground-tertiary">${item.price.toFixed(2)}</div>
            </div>
            {expanded ? <ChevronUp size={13} className="text-foreground-tertiary" /> : <ChevronDown size={13} className="text-foreground-tertiary" />}
          </div>
        </div>
        ...
        <div className="mt-2.5 flex items-center gap-2">
          <div className="flex-1 h-1.5 rounded-full bg-background-tertiary overflow-hidden">
            <div
              ...
              style={{
                width: `${item.esgScore}%`,
                background: item.esgScore >= 70 ? "hsl(142 69% 58%)" : item.esgScore >= 45 ? "hsl(43 96% 56%)" : "hsl(0 72% 71%)",
              }}
            />
          </div>
          <span className="text-[10px] text-foreground-tertiary font-mono shrink-0">{item.esgScore}/100</span>
      ...
      {expanded && (
        <div className="mt-4 space-y-4 animate-fade-up border-t border-border/40 pt-4">
          ...
          {/* Emission breakdown */}
          <div>
            <div className="text-[11px] text-foreground-tertiary mb-2">Emission breakdown</div>
            <div className="flex rounded-xl overflow-hidden h-3 bg-background-tertiary">
              <div className="bg-primary/50" style={{ width: `${item.breakdown.farming}%` }} />
              <div className="bg-primary" style={{ width: `${item.breakdown.processing}%` }} />
              <div
                style={{
                  width: `${item.breakdown.transport}%`,
                  background: item.transport === "air" ? "hsl(0 72% 71%)" : "hsl(43 96% 56%)",
                }}
              />
              <div className="bg-foreground-tertiary/30" style={{ width: `${item.breakdown.packaging}%` }} />
            </div>
          ...
          <div className={`text-[11px] font-medium ${item.inSeason ? "text-primary" : "text-warning"}`}>
            {item.inSeason ? "✓ In season locally" : "✗ Out of local season"}
          </div>

          {item.barcode && (
            <div className="text-[10px] font-mono text-foreground-tertiary/70">Barcode: {item.barcode}</div>
          )}

          {item.localAlt && (
            <div className="p-3 rounded-2xl bg-primary/5 border border-primary/15">
              <div className="text-[10px] text-foreground-tertiary mb-2">🌿 Local alternative</div>
              <div className="flex items-center justify-between mb-2.5">
                <div>
                  <div className="text-[13px] font-semibold text-foreground">{item.localAlt.name}</div>
                  <div className="text-[10px] text-foreground-tertiary">{item.localAlt.origin} · {item.localAlt.distance} km</div>
                </div>
                <div className="text-right">
                  <div className="text-[13px] font-mono font-bold text-primary">{item.localAlt.co2}g CO₂</div>
